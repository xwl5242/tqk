...
import com.quanchong.server.feign.TbGoodsFeignClient;
...
public class TbGoodsJob extends QuartzJobBean {
    ...
    @Autowired
    private TbGoodsFeignClient tbGoodsFeignClient;
    ...
    protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException, RuntimeException {
        ...
        // 删除系统中已经存在的物料信息
        if(tbItemFeignClient.queryCount() != 0){
            optFlag = optFlag && tbItemFeignClient.removeAll();
        }
        // 查询需要默认采集的materialId
        ...
        tbCouponMaterialList.stream().forEach(tbCouponMaterial -> {
            ...
            for(long i=1; i<=tbCouponMaterial.getDefaultCollectNum()/40; i++){
                try {
                    // 调用淘宝客接口一次获取100条记录
                    List<TbItem> tbItemList = tbGoodsFeignClient
                            .queryByMaterialId(tbCouponMaterial.getMaterialId(), i, 40L);
                    // 持久化到数据库
        ...
        optFlag = optFlag && tbItemFeignClient.saveBatch(tbItems);
        if(!optFlag){
